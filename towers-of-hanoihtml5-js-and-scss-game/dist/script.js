...
$(document).ready(function() {
		...
		if (moves > minMoves - 1) {
			if ($tower.eq(1).children().length === disksNum || $tower.eq(2).children().length === disksNum) {
				swal({
					...
					title: 'Xin chúc mừng! Bạn đã thắng',				
					type: 'success',
	...
	function tool(A,C) {		
		a = $(A.selector + '>li:last').remove();		
		C.append(a);	
								
	}
	
	function bot(n,A, B,C)
	{			
		if(n==1){
			tool(A,C); 
			console.log(A.selector,'=>',C.selector)
		}
		else {  
		bot(n - 1, A, C, B);
		bot(1,A,B,C);
	...
	$('.tool').click(function(){	
		bot(disksNum,$("#tower-1"),$("#tower-2"),$("#tower-3"));
	});
	...
	$restart.on('click', function() {
		swal({
				allowEscapeKey: false,
				allowOutsideClick: false,
				title: 'Bạn muốn chơi lại?',
				text: "Tiến trình của bạn sẽ bị mất!",
				type: 'warning',
